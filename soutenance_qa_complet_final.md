...
22. [Backend — Fichiers nodes.py (implémentation des nœuds LangGraph)](#22-backend--fichiers-nodespy-implémentation-des-nœuds-langgraph)
23. [Backend — Routes 3D détaillées (routes_3d.py)](#23-backend--routes-3d-détaillées-routes_3dpy)
24. [Backend — Routes Unity (routes_unity.py)](#24-backend--routes-unity-routes_unitypy)
25. [Backend — Worker configuration (worker.py)](#25-backend--worker-configuration-workerpy)
26. [Frontend — Hook useTaskPolling détaillé](#26-frontend--hook-usetaskepolling-détaillé)
27. [Frontend — Component ModelViewer3D](#27-frontend--component-modelviewer3d)
28. [Hunyuan3D — Architecture détaillée](#28-hunyuan3d--architecture-détaillée)
29. [Services — Détails implémentation](#29-services--détails-implémentation)
30. [Tests — smoke_test.py détails](#30-tests--smoke_testpy-détails)
31. [Déploiement — Docker Compose détails](#31-déploiement--docker-compose-détails)
32. [Métriques & Monitoring](#32-métriques--monitoring)
33. [Glossaire technique](#33-glossaire-technique)
34. [Checklist pré-soutenance](#34-checklist-pré-soutenance)
35. [LangGraph — Récapitulatif d'usage (AJOUT SPÉCIAL)](#35-langgraph--récapitulatif-dusage-ajout-spécial)
36. [Diagnostics — Problème de réutilisation de textures (AJOUT SPÉCIAL)](#36-diagnostics--problème-de-réutilisation-de-textures-ajout-spécial)
...
Endpoint `/api/v1/cache-stats` expose la liste complète pour enrichissement gallery.
...
12. **file_storage_service.py** — upload/storage abstraction
...
## 36. Diagnostics — Problème de réutilisation de textures (AJOUT SPÉCIAL)

### Q36.1 Quel est le problème de texture entre différentes générations ?
**R :** Lorsque vous générez un modèle 3D, puis un autre, la texture du premier modèle est réutilisée dans le second. Cela se produit parce que :

1. **Le service `Hunyuan3DService` est un singleton** qui persiste entre différentes requêtes de génération
2. **Le `MeshRender` conserve l'état entre les appels** - notamment les propriétés de texture et de mesh
3. **Les instances de pipeline de texture ne sont pas réinitialisées** entre les générations successives

### Q36.2 Où se trouve exactement le problème dans le code ?
**R :** Plusieurs points critiques :

1. **Dans `hunyuan3d_service.py`** (lignes 130-132) : 
   ```python
   self.texgen_pipeline: Optional[Any] = None  # Instance partagée entre requêtes
   ```

2. **Dans `pipelines.py`** (ligne 341) :
   ```python
   self.render.load_mesh(mesh)  # Stocke mesh_copy dans l'instance persistante
   ```
   Cela affecte `self.mesh_copy = mesh` dans `MeshRender`

3. **Dans `mesh_render.py`** (lignes 277-288) :
   ```python
   def set_texture(self, tex):
       self.tex = torch.from_numpy(tex).to(self.device)  # Conserve l'état
   ```

4. **Dans `pipelines.py`** (ligne 581) :
   ```python
   self.render.set_texture(texture)  # Fixe la texture dans l'instance
   ```

### Q36.3 Pourquoi les textures du précédent modèle persistent-elles ?
**R :** La méthode `load_mesh()` dans `MeshRender` fait `self.mesh_copy = mesh` (ligne 234) et si un mesh a déjà une texture (par exemple provenant d'une génération précédente), cette texture peut influencer la nouvelle génération. 

De plus, le `set_texture()` stocke l'état dans l'instance `self.tex`, et rien ne réinitialise cet état entre les appels successifs.

### Q36.4 Quelle est la racine du problème d'architecture ?
**R :** 
- Le service `Hunyuan3DService` est conçu comme un singleton pour des raisons de performance (chargement coûteux des modèles)
- Mais cela signifie que l'objet `MeshRender` dans `Hunyuan3DPaintPipeline` persiste entre les requêtes
- L'état de texture/mesh n'est pas correctement réinitialisé entre les générations

### Q36.5 Comment corriger ce problème ?
**R :** Plusieurs approches possibles :

1. **Réinitialisation de l'état texture entre les générations** :
   ```python
   def __call__(self, mesh, image, user_views=None):
       # Nettoyer l'état précédent
       if hasattr(self.render, 'tex'):
           delattr(self.render, 'tex')
       
       # ... code de génération ...
   ```

2. **Création d'instance MeshRender temporaire** :
   - Créer un nouvel objet `MeshRender` pour chaque génération
   - Ou réinitialiser complètement l'état du renderer avant chaque génération

3. **Mise en place d'une méthode `reset()`** dans `MeshRender` :
   ```python
...
### Q36.6 Comment diagnostiquer ce genre de problème ?
**R :** Méthodes de diagnostic :
1. **Logguer l'état interne** du renderer avant chaque génération
2. **Comparer les propriétés de texture** entre différentes générations
3. **Vérifier la réinitialisation des états partagés** entre les requêtes
4. **Utiliser un profiler mémoire** pour identifier les objets persistants

---

**Bon soutenance !** 🚀

*Mis à jour : 2026-06-05 — Couvre Frontend (18 composants, 3 hooks), Backend (12 services, 3 routers, 6 Celery tasks), Unity (SpawnBridge, SpawnRequest), Docker, Makefile, tests, monitoring, LangGraph usage approfondi, et **problème de cache texture**.*
