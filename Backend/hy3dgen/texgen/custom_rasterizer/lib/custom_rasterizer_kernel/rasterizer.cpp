        ...
        for (int py = y_min; py < y_max + 1; ++py) {
            ...
            if (isBarycentricCoordInBounds(baryCentricCoordinate)) {
                ...
                if (depth < depth_thres)
                    continue;
                zbuffer[pixel] = std::min(zbuffer[pixel], (INT64)token);
            }
        }
    }
}
    ...
    int width, int height, float occlusion_truncation, int use_depth_prior)
{
    ...
    if (device_id == -1)
        ...
        // On CPU-only builds, move tensors to CPU and use the CPU path
        return rasterize_image_cpu(V.cpu(), F.cpu(), D.cpu(), width, height, occlusion_truncation, use_depth_prior);
    }
#endif
}
