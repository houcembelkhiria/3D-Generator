...
inline int pos2key(float* p, int resolution) {
    int x = (p[0] * 0.5f + 0.5f) * resolution;
    int y = (p[1] * 0.5f + 0.5f) * resolution;
    int z = (p[2] * 0.5f + 0.5f) * resolution;
...
inline void key2pos(int key, int resolution, float* p) {
    ...
    p[0] = ((x + 0.5f) / resolution - 0.5f) * 2;
    p[1] = ((y + 0.5f) / resolution - 0.5f) * 2;
    p[2] = ((z + 0.5f) / resolution - 0.5f) * 2;
}
...
inline void key2cornerpos(int key, int resolution, float* p) {
    ...
    int y = key / resolution % resolution;
    int z = key % resolution;
    p[0] = ((x + 0.75f) / resolution - 0.5f) * 2;
    p[1] = ((y + 0.25f) / resolution - 0.5f) * 2;
    p[2] = ((z + 0.75f) / resolution - 0.5f) * 2;
}
    ...
    int* output_indices)
{
    ...
    int cj = 0;
    if (dim == 0) {
        ci = (pos[1]/2+0.5f)*height;
        cj = (pos[0]/2+0.5f)*width;
    }
    else if (dim == 1) {
        ci = (pos[1]/2+0.5f)*height;
        cj = (pos[2]/2+0.5f)*width;
    }
    else {
        ci = (-pos[2]/2+0.5f)*height;
        cj = (pos[0]/2+0.5f)*width;
    }
