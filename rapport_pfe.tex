...
dans l'éditeur Unity via le protocole \textbf{MCP} (\textit{Model Context
Protocol}). La méthodologie \textbf{CRISP-DM} structure le projet en six
...
FastAPI, MCP, Unity, ChromaDB, microservice agentique.
...
automatically injects them into Unity Editor via the \textbf{Model Context
Protocol (MCP)}. The \textbf{CRISP-DM} methodology structures the project
...
FastAPI, MCP, Unity, ChromaDB, agentic microservice.
...
et leur injection dans Unity Editor via le protocole MCP.
...
\begin{itemize}[leftmargin=2.5cm,label=\textbullet]
  ...
  \item \textbf{Chapitre 3} — Modélisation et architecture : modèles IA, intégration MCP.
...
directement dans Unity Editor via le protocole MCP --- sans aucune
...
\subsection{Le paradigme MCP appliqué à l'intégration Unity}
Le Model Context Protocol (MCP), proposé par Anthropic en 2024, standardise
la connexion entre assistants IA et outils locaux via un protocole client/serveur.
Le projet s'inspire de ce paradigme : le backend Python joue le rôle de
\textbf{client} (il émet les requêtes d'action via \texttt{POST /unity/spawn})
et le script éditeur \texttt{SpawnBridge.cs} joue le rôle de \textbf{serveur
d'outils} (il exécute l'action \texttt{spawn}). Le transport retenu est
un dossier de fichiers JSON partagé --- plus léger qu'un WebSocket et sans
dépendance réseau. Un protocole MCP bidirectionnel complet figure parmi les
...
\begin{itemize}
  ...
  \item Injection automatique dans Unity via MCP (aucun clic requis).
...
US-4.3 & MCP Unity      & Outil Unity : SpawnBridge (GLB → scène Unity) & S5 \\
...
US-5.3 & Doc            & Rédaction mémoire + documentation MCP & S7 \\
...
\chapterintro{%
  \begin{enumerate}[noitemsep,leftmargin=*]
    ...
    \item Intégration Unity via MCP (SpawnBridge, glTFast, URI)
...
\begin{tikzpicture}[
  ...
  \draw[arr](be)--node[left,lbl]{SpawnRequest\\JSON (MCP)}(un);
\end{tikzpicture}
...
\section{Intégration Unity via MCP}
\label{sec:unity-mcp}
...
retenus, le pipeline asynchrone et le mécanisme MCP pour Unity. Le chapitre
...
\begin{itemize}
  ...
  \item Injection automatique dans Unity via le protocole \textbf{MCP} et
...
\begin{enumerate}
  ...
  \item Protocole MCP bidirectionnel complet (WebSocket).
...
\begin{enumerate}
  ...
  \item Anthropic MCP --- \url{https://modelcontextprotocol.io} --- consulté en 2025.
