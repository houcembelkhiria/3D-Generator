...
Les principales solutions de génération 3D depuis une image ou un texte sont
comparées ci-dessous selon leur architecture, leurs performances matérielles et
leurs limites pratiques.

{\footnotesize\renewcommand{\arraystretch}{1.2}
\begin{longtable}{@{}p{2.3cm}p{2.2cm}p{1.3cm}p{1.1cm}p{2.6cm}p{3.8cm}@{}}
\caption{Comparatif des solutions de génération 3D existantes}
\label{tab:existant}\\
\toprule
\theadrow
\textbf{Outil} & \textbf{Approche} & \textbf{Temps} & \textbf{VRAM} &
\textbf{Points forts} & \textbf{Limites / Contraintes} \\
\midrule
\endfirsthead
\toprule
\theadrow
...
TripoSR~\cite{triposr}
  & NeRF / Transformer & $<$2\,s & 4\,Go
  & Ultra-rapide, open source
  & Qualité géom. faible, artefacts sur faces cachées \\[2pt]
InstantMesh~\cite{instantmesh}
  & LRM sparse-view & $\sim$10\,s & 8\,Go
  & Équilibre qualité / vitesse, open source
  & Texture limitée, pas de PBR \\[2pt]
Point-E~\cite{pointe}
  & Diffusion nuage de points & $\sim$10\,s & 6\,Go
  & Text-to-3D, open source
  & Résolution très faible, maillage approximatif \\[2pt]
Shap-E~\cite{shape_e}
  & Diffusion implicite & $\sim$15\,s & 8\,Go
  & Text-to-3D natif, open source
  & Maillage grossier, pas de texture PBR \\[2pt]
Zero123++~\cite{zero123pp}
  & Diffusion multi-vues & $\sim$30\,s & 12\,Go
  & Bonne précision géométrique
  & GPU haute gamme requis, pas de pipeline texte \\[2pt]
Meshy.ai
  & API cloud propriétaire & Variable & Cloud
  & Interface simple, PBR supporté
  & Payant, données transmises vers serveurs tiers \\[2pt]
Luma AI Genie
  & Diffusion vidéo / 3D & Variable & Cloud
  & Qualité visuelle élevée
  & Cloud uniquement, pas d'API libre, payant \\[4pt]
\midrule
\textbf{Notre système}~\cite{hunyuan3d2024}
  & \textbf{DiT + VAE 3D (Hunyuan3D) + LangGraph}
  & \textbf{45\,s -- 4\,min} & \textbf{5--6\,Go}
  & \textbf{Multi-modal, PBR, Unity, PDF$\rightarrow$3D, cache vect.}
  & Génération plus lente ; GPU requis \\
\end{longtable}
}
...
La matrice ci-dessous compare les capacités fonctionnelles de notre système face
aux solutions existantes. Les critères en \textbf{gras} représentent des
contributions originales absentes de l'écosystème open-source actuel.

{%
\def\yes{{\color{crispgreen}\checkmark}}%
\def\no{{\color{red!60}$\times$}}%
\def\pt{{\color{orange!70}$\sim$}}%
\footnotesize\renewcommand{\arraystretch}{1.35}%
\begin{table}[H]
\centering
\caption{Matrice de positionnement — critères différenciants}
\label{tab:positionnement}
\begin{tabular}{@{}p{4.1cm}
  >{\centering\arraybackslash}p{1.35cm}
  >{\centering\arraybackslash}p{1.35cm}
  >{\centering\arraybackslash}p{1.35cm}
  >{\centering\arraybackslash}p{1.35cm}
  >{\centering\arraybackslash}p{1.35cm}
  >{\columncolor{tekupblue!10}\centering\arraybackslash}p{1.35cm}@{}}
\toprule
\theadrow
\textbf{Critère} &
\makecell[c]{\textbf{Tripo}\\\textbf{SR}} &
\makecell[c]{\textbf{Instant}\\\textbf{Mesh}} &
\textbf{Shap-E} &
\makecell[c]{\textbf{Zero}\\\textbf{123++}} &
\textbf{Meshy} &
\makecell[c]{\bfseries\color{tekupblue}Notre\\syst.} \\
\midrule
Open source                              & \yes & \yes & \yes & \yes & \no  & \yes \\
Exécution locale                         & \yes & \yes & \yes & \yes & \no  & \yes \\
Image $\rightarrow$ 3D                   & \yes & \yes & \pt  & \yes & \yes & \yes \\
Texte $\rightarrow$ 3D                   & \no  & \no  & \yes & \no  & \yes & \yes \\
Multi-vues $\rightarrow$ 3D              & \no  & \yes & \no  & \yes & \yes & \yes \\
\textbf{Document PDF $\rightarrow$ 3D}  & \no  & \no  & \no  & \no  & \no  & \textbf{\yes} \\
Matériaux PBR                            & \no  & \pt  & \no  & \no  & \yes & \yes \\
\textbf{Intégration Unity Editor}       & \no  & \no  & \no  & \no  & \no  & \textbf{\yes} \\
\textbf{Cache vectoriel}                & \no  & \no  & \no  & \no  & \no  & \textbf{\yes} \\
API REST asynchrone                      & \no  & \no  & \no  & \no  & \yes & \yes \\
\textbf{Pipeline agentique (LangGraph)} & \no  & \no  & \no  & \no  & \no  & \textbf{\yes} \\
\bottomrule
\end{tabular}
\end{table}
}

Notre système se distingue par \textbf{quatre contributions absentes} de toutes
les solutions recensées : (1)~un pipeline automatique \textbf{document PDF /
email $\rightarrow$ asset 3D}, sans intervention manuelle grâce à LangGraph et
un LLM local ; (2)~une \textbf{injection directe dans Unity Editor} via un
protocole JSON partagé, sans dépendance réseau ; (3)~un \textbf{cache vectoriel}
(ChromaDB + embeddings CLIP/DINO) éliminant les régénérations redondantes ;
et (4)~un \textbf{pipeline agentique} orchestrant l'ensemble du workflow de
façon autonome. Ces capacités font de ce microservice un outil de production
directement intégrable dans un contexte de développement de jeu vidéo.
...
\label{sec:modeles-ia}

Le pipeline Hunyuan3D s'appuie sur une famille de modèles spécialisés couvrant l'intégralité
du traitement : de la génération de forme brute jusqu'à la synthèse de matériaux physiquement
réalistes (PBR), en passant par le prétraitement d'image et la reconstruction de scènes
immersives. Le tableau~\ref{tab:modeles-hunyuan} en dresse la vue d'ensemble comparative,
regroupée par rôle fonctionnel.

{\renewcommand{\arraystretch}{1.2}\footnotesize
\begin{longtable}{@{}p{3.5cm}p{1.6cm}p{1.0cm}p{1.0cm}p{2.8cm}p{3.5cm}@{}}
\caption{Famille des modèles Hunyuan3D — vue d'ensemble}
\label{tab:modeles-hunyuan}\\
\toprule
\theadrow
\textbf{Modèle} & \textbf{Rôle principal} & \textbf{Param.} &
\textbf{VRAM min.} & \textbf{Entrées / Sorties} &
\textbf{Architecture \& spécificités} \\
\midrule
\endfirsthead
\multicolumn{6}{c}{\itshape (suite — Famille des modèles Hunyuan3D)}\\[2pt]
\toprule
\theadrow
\textbf{Modèle} & \textbf{Rôle principal} & \textbf{Param.} &
\textbf{VRAM min.} & \textbf{Entrées / Sorties} &
...
\midrule
\endhead
\multicolumn{6}{r}{\itshape Suite page suivante $\rightarrow$}
\endfoot
\bottomrule
\endlastfoot

%% ---- Génération de formes ----
\multicolumn{6}{l}{\bfseries\color{tekupblue}%
  Génération de formes (Hunyuan3D-DiT / Shape)} \\[2pt]
Hunyuan3D-DiT-v2-0
  & Génération de formes & 1,1\,B & $\sim$6\,Go
  & Image / Texte $\rightarrow$ Maillage 3D (.glb, .obj)
  & \textit{Flow-based Diffusion Transformer} ; modèle de base pour la géométrie. \\[2pt]
Hunyuan3D-DiT-v2-0-Fast
  & Génération de formes & 1,1\,B & $\sim$6\,Go
  & Image / Texte $\rightarrow$ Maillage 3D
  & Distillation \textit{Guidance} : inférence $2\times$ plus rapide que le Standard. \\[2pt]
Hunyuan3D-DiT-v2-0-Turbo
  & Génération de formes & 1,1\,B & $\sim$6\,Go
  & Image / Texte $\rightarrow$ Maillage 3D
  & Distillation \textit{Step} (FlashVDM) : étapes de calcul réduites drastiquement. \\[2pt]
Hunyuan3D-DiT-v2-mini (Std / Fast / Turbo)
  & Génération de formes & 0,6\,B & $\sim$5\,Go
  & Image / Texte $\rightarrow$ Maillage 3D
  & Version allégée, optimisée GPU grand public à mémoire contrainte. \\[2pt]
Hunyuan3D-DiT-v2-mv (Std / Fast / Turbo)
  & Génération multi-vues & 1,1\,B & $\sim$6\,Go
  & Images multi-angles $\rightarrow$ Maillage 3D
  & Angles traités simultanément ; supprime les artefacts sur faces cachées. \\[2pt]
Hunyuan3D-Shape-v2-1 (LATTICE)
  & Haute fidélité & 3,3\,B & $\sim$10\,Go
  & Image $\rightarrow$ Maillage haute densité
  & Représentation hybride \textbf{VoxSet} (Voxel + Vector Set) ; surfaces ultra-lisses. \\[4pt]
\midrule

%% ---- Texture / Matériaux ----
\multicolumn{6}{l}{\bfseries\color{tekupblue}%
  Synthèse de textures et matériaux (Hunyuan3D-Paint)} \\[2pt]
Hunyuan3D-Paint-v2-0 (\& Turbo)
  & Textures RGB & 1,3\,B & $\sim$10\,Go
  & Maillage + Image $\rightarrow$ Texture RGB
  & Priors de diffusion géométrique ; coloriage sans coutures sur maillage vierge. \\[2pt]
Hunyuan3D-Paint-v2-1
  & Matériaux PBR & 2,0\,B & $\sim$21\,Go
  & Maillage + Image (sans ombre) $\rightarrow$ Cartes PBR
  & Attention multi-vues + RoPE 3D ; génère Albédo, Métallique, Rugosité, Normales. \\[2pt]
Hunyuan3D-Delight-v2-0
  & Prétraite\-ment image & 1,3\,B & Variable
  & Image brute $\rightarrow$ Image d'albédo
  & Suppression mathématique des ombres portées et des reflets lumineux. \\[4pt]
\midrule

%% ---- Scènes immersives ----
\multicolumn{6}{l}{\bfseries\color{tekupblue}%
  Reconstruction et scènes immersives (HunyuanWorld)} \\[2pt]
HunyuanWorld-1.0
  & Scènes immersives & N/A & Variable
  & Image / Texte + labels sémantiques $\rightarrow$ Monde interactif (DRC)
  & Panoramas 360° en couches maillées via \textit{onion-peeling} ; premier plan / arrière-plan interactifs. \\[2pt]
HunyuanWorld-Mirror
  & Recons\-truction
    univer\-selle & N/A & Variable
  & Vidéo / Image + priors $\rightarrow$ Gaussiennes 3D / Nuages de points
  & Modèle \textit{feed-forward} ; profondeur, normales et paramètres caméra extraits en un seul passage. \\
\end{longtable}
}
...
conçu pour orchestrer des \textbf{workflows à état} sous forme de
graphes orientés. Contrairement à une simple chaîne d'appels séquentielle,
LangGraph permet de définir des \textbf{nœuds} (étapes de traitement)
reliés par des \textbf{arêtes conditionnelles} et organisés en
sous-graphes réutilisables ; selon l'état partagé après l'exécution
d'un nœud, le routeur conditionnel peut bifurquer, boucler ou
quitter le sous-graphe pour rendre la main au graphe parent.
...
\toprule
\textbf{Aspect} & \textbf{Pipeline linéaire classique} & \textbf{Pipeline LangGraph (notre implémentation)} \\\midrule
Flux              & Fixe, séquentiel                & Graphe conditionnel + sous-graphes \\
Reprise           & Aucune (relance à zéro)         & Checkpointer SQLite : reprise depuis le dernier nœud terminé \\
Progrès en direct & Aucun                           & Streaming d'événements (1 callback par nœud) \\
Reprises automatiques & Manuelles (nouveau POST)    & Retry borné par routeur + fallback hand-crafted \\
Traçabilité       & Logs texte                      & État dictionnaire typé sérialisable (JSON) \\
HITL              & Impossible sans refacto         & \texttt{interrupt\_after} natif (revue humaine) \\
Async             & Bloquant                        & \texttt{ainvoke} / \texttt{astream} disponibles \\
\bottomrule
\end{tabularx}
\end{table}

Le module \texttt{Backend/app/pipeline/} implémente ce graphe avec
cinq nœuds top-level et deux \textbf{sous-graphes} compilés
encapsulant les boucles de retry (\texttt{graph.py}) :

\begin{enumerate}
  \item \textbf{parse\_document} --- lecture du PDF ou email via \texttt{unstructured}.
  \item \textbf{validate\_parsed\_document} --- vérification que le parsing
    a produit du contenu utilisable (signale les échecs OCR / documents
    image-only avant de gaspiller des appels LLM).
  \item \textbf{spec\_extraction} (sous-graphe) --- contient
    \texttt{extract\_spec\_llm} → \texttt{validate\_spec} avec retry
    (max 3 tentatives) ou \texttt{build\_fallback\_spec}.
  \item \textbf{mesh\_generation} (sous-graphe) --- contient
    \texttt{generate\_mesh} → \texttt{validate\_mesh} avec retry
    (max 2 tentatives) avant de poursuivre.
  \item \textbf{store\_result} --- persistance dans \texttt{gallery\_db}
    et retour des métadonnées du modèle.
\end{enumerate}

\paragraph{Fonctionnalités LangGraph mobilisées.} L'implémentation exploite
l'ensemble des primitives offertes par le framework, et non un sous-ensemble
minimal :

\begin{itemize}
  \item \texttt{StateGraph} typé avec \texttt{Pipeline3DState} (TypedDict)
    et reducer \texttt{Annotated[List[str], operator.add]} sur le champ
    \texttt{errors} pour accumuler les avertissements entre nœuds.
  \item \texttt{add\_conditional\_edges} pour les routeurs de retry et
    de fallback, ramenant en arrière dans le sous-graphe tant que la
    limite n'est pas atteinte.
  \item Sous-graphes compilés et insérés comme nœuds dans le graphe parent,
    isolant les retries internes du flux principal et améliorant la lisibilité.
  \item \texttt{SqliteSaver} (checkpointer persistant) avec WAL :
    chaque transition d'état est écrite sur disque, permettant la
    reprise via \texttt{POST /api/v1/resume-pipeline/\{thread\_id\}}
    après un crash du worker Celery.
  \item \texttt{interrupt\_after} optionnel pour le mode
    \emph{human-in-the-loop} : un opérateur peut inspecter l'état
    intermédiaire et reprendre l'exécution après validation manuelle.
  \item Streaming via \texttt{pipeline.stream(subgraphs=True)} : chaque
    transition de nœud déclenche un callback qui met à jour l'état
    Celery (\texttt{self.update\_state}) avec \texttt{current\_node} et
    \texttt{recent\_errors}, exposés au frontend.
  \item Variantes asynchrones \texttt{ainvoke} et \texttt{astream}
    disponibles pour migration future vers FastAPI async natif.
  \item Timeouts par nœud via gestionnaire de contexte \texttt{\_node\_timeout}
    (\texttt{SIGALRM}) avec budgets configurables par variable
    d'environnement : \texttt{LG\_TIMEOUT\_LLM=120\,s},
    \texttt{LG\_TIMEOUT\_MESH=1200\,s}.
\end{itemize}
...
L'identifiant de tâche Celery sert directement de \texttt{thread\_id}
LangGraph, ce qui unifie les deux mécanismes de suivi : un même
\texttt{task\_id} suffit pour interroger l'état d'avancement
(\texttt{GET /task/\{id\}}) et pour relancer une exécution
(\texttt{POST /resume-pipeline/\{id\}}).

\paragraph{Articulation Celery $\leftrightarrow$ LangGraph.} Les deux
frameworks sont complémentaires et non redondants. LangGraph définit
\emph{quoi} faire (la structure du workflow, le retry, le fallback,
la sérialisation d'état). Celery décide \emph{où et quand} l'exécuter
(file dédiée GPU, isolation par processus, redémarrage propre du
worker, limites de temps strictes). Le tableau~\ref{tab:celery-langgraph}
résume la répartition des responsabilités.
...
\begin{table}[H]
\centering
\caption{Répartition Celery $\leftrightarrow$ LangGraph}
\label{tab:celery-langgraph}
\begin{tabularx}{\linewidth}{@{}lXX@{}}
\toprule
\textbf{Préoccupation} & \textbf{Celery} & \textbf{LangGraph} \\\midrule
File / queue              & \yes & \no \\
Isolation processus       & \yes & \no \\
Retry intra-workflow      & \no  & \yes (routeurs) \\
Fallback hand-crafted     & \no  & \yes (\texttt{build\_fallback\_spec}) \\
Reprise post-crash        & \no  & \yes (checkpointer) \\
Limite de temps globale   & \yes & \no \\
Limite de temps par nœud  & \no  & \yes (\texttt{\_node\_timeout}) \\
Progression en direct     & \no  & \yes (\texttt{stream()}) \\
Stockage du résultat      & \yes (Redis) & \yes (checkpointer) \\
Annulation propre         & \yes (\texttt{revoke + SIGTERM}) & \no \\
\bottomrule
\end{tabularx}
\end{table}
}%

\paragraph{Migration des endpoints GPU directs vers Celery.}
Avant la refactorisation finale, les trois modes de génération directe
(\texttt{image-to-3d}, \texttt{text-to-3d}, \texttt{multi\-view-to-3d})
ainsi que \texttt{retexture} s'exécutaient dans le processus FastAPI
via \texttt{threading.Thread(daemon=True)}, en parallèle des tâches
Celery réservées au pipeline LangGraph. Cette dualité posait quatre
problèmes : (i)~un redémarrage de FastAPI tuait tous les jobs en
cours sans possibilité de reprise ; (ii)~aucune file ne sérialisait
les requêtes GPU concurrentes, exposant le système à des OOM CUDA ;
(iii)~les dictionnaires en mémoire \texttt{\_progress} et
\texttt{\_pending\_results} grossissaient sans persistance et étaient
plafonnés artificiellement à 500 entrées ; (iv)~les workers Celery
configurés pour la file \texttt{3d\_generation} étaient inutilisés
pour ces endpoints.

La refactorisation consolide tous les modes derrière Celery : le module
\texttt{Backend/app/tasks\_3d.py} expose les quatre tâches GPU
(\texttt{image\_to\_3d\_task}, \texttt{text\_to\_3d\_task},
\texttt{multiview\_to\_3d\_task}, \texttt{retexture\_task}) qui
réalisent les appels au service Hunyuan3D et publient leur état via
\texttt{self.update\_state(\{stage, pct, \ldots\})}. Le WebSocket
\texttt{/api/v1/ws/generation/\{uid\}} lit ces états depuis
\texttt{AsyncResult.info} et transmet la même structure JSON
\texttt{\{stage, pct, ...\}} attendue par le frontend, garantissant
l'absence de régression sur le protocole.

\paragraph{Validation par test de fumée.} Un script
\texttt{Backend/tests/smoke\_test.py} (260 lignes, sans pytest)
exerce l'ensemble du surface API et la chaîne Celery sans nécessiter
de Redis ni de GPU. Il utilise le mode \texttt{task\_always\_eager=True}
de Celery pour exécuter les tâches en ligne et substitue le service
Hunyuan3D par un objet mock. Les 23 vérifications couvrent les
quatre endpoints \texttt{/async}, l'enregistrement des tâches
Celery, la configuration de production, le retrait de
l'endpoint legacy \texttt{/upload}, le schéma \texttt{Pipeline3DState}
sans champ obsolète, ainsi que les endpoints
\texttt{/pipeline-state} et \texttt{/resume-pipeline}. L'exécution
prend moins de cinq secondes et constitue un filet de sécurité
pour toute future modification de la chaîne d'exécution.
...
Celery~\cite{celery} & File de tâches & Tous les modes de génération (4 endpoints directs + pipeline LangGraph) exécutés via workers Celery dédiés, file \texttt{3d\_generation}, \texttt{prefetch\_multiplier=1} pour sérialiser le GPU \\
