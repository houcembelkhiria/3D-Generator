...
class MeshRender():
    def __init__(
        ...
        if self.raster_mode == 'cr':
            from hy3dgen.texgen.custom_rasterizer import custom_rasterizer as cr
            self.raster = cr
            # Force CPU for custom rasterizer on Mac MPS
            if str(self.device).startswith('mps'):
                print(f"[MPS Debug] MeshRender detected MPS device ({self.device}), but custom rasterizer requires CPU/CUDA. Staying on CPU for rasterization.")
                self.device = 'cpu'
        else:
    ...
    def raster_rasterize(self, pos, tri, resolution, ranges=None, grad_db=True):
        ...
        if self.raster_mode == 'cr':
            ...
            # Ensure tensors are on the correct device (CPU for 'cr' on Mac)
            if self.raster_mode == 'cr' and str(pos.device).startswith('mps'):
                pos = pos.cpu()
                tri = tri.cpu()
    ...
    def raster_interpolate(self, uv, rast_out, uv_idx, rast_db=None, diff_attrs=None):
        ...
        if self.raster_mode == 'cr':
            ...
            if str(uv.device).startswith('mps'):
                uv = uv.cpu()
            if str(findices.device).startswith('mps'):
                findices = findices.cpu()
            if str(barycentric.device).startswith('mps'):
                barycentric = barycentric.cpu()
            if str(uv_idx.device).startswith('mps'):
                uv_idx = uv_idx.cpu()
    ...
    def to(self, device):
        if str(device).startswith('mps'):
            print(f"[MPS Debug] MeshRender.to({device}) called. Forcing CPU fallback for stability.")
            device = 'cpu'
        self.device = device
