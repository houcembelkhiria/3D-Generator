...
class HunyuanPaintPipeline(StableDiffusionPipeline):
    ...
    def _execution_device(self):
        r"""
        Returns the device on which the pipeline is executed.
        """
    ...
    def __call__(
        ...
        **cached_condition,
        ...
        print(f"[MPS Debug] Pipeline __call__ - Execution Device: {device}, VAE Dtype: {dtype}")
        
        # Ensure sub-modules are on the correct device
        if str(device) == 'cpu':
           self.vae.to(device)
           self.unet.to(device)
        ...
        image_vae = image_vae.to(device=device, dtype=dtype)
        ...
        def convert_pil_list_to_tensor(images):
            ...
            for batch_imgs in images:
                ...
                for pil_img in batch_imgs:
                    ...
                    img = torch.from_numpy(img).permute(2, 0, 1).unsqueeze(0).contiguous().to(device=device, dtype=dtype)
